...
| Full app boot, local workspace creation, quick capture, save, recent workspace loading, desktop attachment bridge, and conflict resolution | `src/App.test.tsx` |
...
| Local Git initialization, no-op saves, commit persistence, recent workspaces, all managed-file additions/updates/deletions, credential exclusion, attachment storage security, remote clone/sync/offline safety, clean multi-file merges, simultaneous JSON/attachment conflicts, modify/delete conflicts, blocked saves during conflicts, and local/remote resolution | `electron/git-service.test.ts` |
